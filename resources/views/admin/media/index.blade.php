...
@section("content")
    <div class="card mb-5 mb-xl-8 mt-5">
        ...
        <div class="card-header border-0 pt-5">
            <h3 class="card-title align-items-start flex-column">
                <span class="card-label fw-bolder fs-3 mb-1">Medya Yönetimi</span>
            </h3>
            <div class="card-toolbar">
                <a href="{{ route('manager.medias.create') }}" class="btn btn-sm btn-light-primary">
        ...
        <div class="card-body py-3">
            ...
            <div class="table-responsive">
                <!--begin::Table-->
                <table class="table align-middle gs-0 gy-4" id="media_table">
                    <!--begin::Table head-->
                    <thead>
                    <tr class="fw-bolder text-muted bg-light">
                        <th class="ps-4 min-w-125px rounded-start">Başlık</th>
                        <th class="ps-4 min-w-125px">Gösterim Yerleri</th>
                    ...
                    @foreach($items as $item)
                        <tr>
                            <td>
                                <div class="d-flex align-items-center">
                                    <div class="symbol symbol-50px me-5">
                                        <img src="{{ route('storage.images', $item->media_poster) }}" class=""
                                             alt="{{ $item->media_title }}"/>
                                    </div>
                                    <div class="d-flex justify-content-start flex-column">
                                        <span class="fw-bold d-block fs-7">{{ $item->media_title }}</span>
                                    </div>
                                </div>
                            </td>
                            <td>
                                <span class="fw-bold d-block fs-7">{{ implode(" | ", $item->categories()->pluck('name')->toArray()) }}</span>
                            </td>
                            <td class="d-none">
                                <span class="fw-bold d-block fs-7">{{ strip_tags($item->media_description) }}</span>
                            </td>
                            <td>
                                <span
                                    class="badge {{ $item->media_is_visible ? "badge-light-success" : "badge-light-danger" }} fs-7 fw-bold">{{ $item->media_is_visible ? "Aktif" : "Pasif" }}</span>
                            </td>
                            <td>
                            ...
                            <td class="text-end">
                                <a href="{{ route('manager.medias.gallery', $item->id) }}"
    ...
    <g stroke="none" stroke-width="1" fill="none" fill-rule="evenodd">
        ...
        <path d="M10.782158,17.5100514 L15.1856088,14.5000448 C15.4135806,14.3442132 15.4720618,14.0330791 15.3162302,13.8051073 C15.2814587,13.7542388 15.2375842,13.7102355 15.1868178,13.6753149 L10.783367,10.6463273 C10.5558531,10.489828 10.2445489,10.5473967 10.0880496,10.7749107 C10.0307022,10.8582806 10,10.9570884 10,11.0582777 L10,17.097272 C10,17.3734143 10.2238576,17.597272 10.5,17.597272 C10.6006894,17.597272 10.699033,17.566872 10.782158,17.5100514 Z" fill="#000000"/>
                                ...
                                <a href="{{ route('manager.medias.edit', $item->id) }}"
                                   class="btn btn-icon btn-bg-light btn-active-color-primary btn-sm me-1">
                                    <!--begin::Svg Icon | path: icons/duotone/Communication/Write.svg-->
                                ...
                                <form style="display: contents"
                                      action="{{ route("manager.medias.destroy", $item->id) }}" method="post">
                                    @method("delete")
                                    @csrf
                                    <button onclick="return confirm('{{ $item->media_title }} içeriğini silmek istediğinize emin misiniz?')" type="submit"
            ...
            buttons: [
                {
                    ...
                    title: 'Medya Raporu - {{ now()->format('d-m-Y H.i.s') }}',
                    exportOptions: {
