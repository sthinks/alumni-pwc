        ...
        <div class="col-lg-12">
            ...
            <div class="card card-custom gutter-b example example-compact">
                <div class="card-header">
                    <h3 class="card-title">Yeni Medya Ekle</h3>
                </div>
                <form class="form" method="post" action="{{ route('manager.medias.store') }}" enctype="multipart/form-data">
                    @csrf
                    <div class="card-body">
                        <div class="mb-10">
                            <label for="media_title" class="required form-label">Medya Başlığı</label>
                            <input id="media_title" value="{{ old("media_title") }}" name="media_title" type="text" class="form-control form-control-solid" required/>
                        </div>
                        <div class="mb-10">
                            ...
                            <div class="checkbox-inline">
                                @foreach($categories as $category)
                                    <label class="checkbox p-3">
                                        <input type="checkbox" name="media_category[]" value="{{ $category->id }}" />
                                        <span></span>
                                        {{ $category->name }}
                                    </label>
                                @endforeach
                        ...
                        <div class="mb-10">
                            ...
                            <textarea id="media_abstract" name="media_abstract" class="form-control form-control-solid">{{ old("media_abstract") }}</textarea>
                        </div>
                        <div class="mb-10">
                            <label for="media_description_id" class="required form-label">Medya Açıklaması</label>
                            <textarea id="media_description_id" name="media_description" class="form-control form-control-solid">{{ old("media_description") }}</textarea>
                        </div>
                        <div class="mb-10">
                            <label for="media_embed" class="form-label">Video linkleri</label>
                            <textarea id="media_embed" name="media_embed" class="form-control form-control-solid">{{ old("media_embed") }}</textarea>
                            <small>Video linkerini virgül ile ayırarak giriniz.</small>
                        </div>
                        <div class="mb-10">
                            <label for="media_poster" class="required form-label">Medya Kapak Görseli</label>
                            <input id="media_poster" name="media_poster" type="file" class="form-control form-control-solid" required/>
                        ...
                        <div class="mb-10">
                            <label for="media_gallery" class="form-label">Medya Görselleri</label>
                            <input id="media_gallery" name="media_gallery[]" type="file" class="form-control form-control-solid" multiple />
                        </div>
                        <div class="row mb-10">
                            <!--begin::Label-->
                            ...
                            <div class="col-lg-9 d-flex align-items-center">
                                <div class="form-check form-check-solid form-switch fv-row">
                                    <input name="media_is_visible" class="form-check-input w-45px h-30px"
