...
@section("title", sprintf("%s - Düzenle", $event->event_title))
@section("content")
    <div class="row">
        <div class="col-lg-12">
            <!--begin::Card-->
            <div class="card card-custom gutter-b example example-compact">
                <div class="card-header">
                    <h3 class="card-title">{{ $event->event_title }} - Düzenle</h3>
                </div>
                <form class="form" method="post" action="{{ route('manager.events.update', $event->id) }}" enctype="multipart/form-data">
                    @csrf
                    @method("PUT")
                    <div class="card-body">
                        <div class="mb-10">
                            <label for="event_title" class="required form-label">Etkinlik Başlığı</label>
                            <input id="event_title" value="{{ $event->event_title }}" name="event_title" type="text" class="form-control form-control-solid" required/>
                        </div>
                        <div class="mb-10">
                            <label for="event_abstract" class="form-label">Etkinlik Kısa Açıklaması</label>
                            <textarea id="event_abstract" name="event_abstract" class="form-control form-control-solid">{{ $event->event_abstract }}</textarea>
                        </div>
                        <div class="mb-10">
                            <label for="event_description" class="required form-label">Etkinlik Metni</label>
                            <textarea id="event_description" name="event_description" class="form-control form-control-solid">{{ $event->event_description }}</textarea>
                        </div>
                        <div class="mb-10">
                            <label for="event_poster" class="required form-label">Etkinlik Görseli</label>
                            <input id="event_poster" name="event_poster" type="file" class="form-control form-control-solid" />
                            <p><a href="{{ route('storage.images', $event->event_poster) }}" target="_blank">Mevcut görseli görmek için tıklayınız</a></p>
                        ...
                        <div class="mb-10">
                            ...
                            <select id="event_city" name="event_city" class="form-control selectpicker">
                                @foreach($cities as $city)
                                    <option value="{{ $city->id }}" @if($event->event_city === $city->id)) selected @endif>{{ $city->city }}</option>
                                @endforeach
                            </select>
                        </div>
                        <div class="mb-10">
                            <label for="event_venue" class="required form-label">Etkinlik Adresi</label>
                            <input id="event_venue" value="{{ $event->event_venue }}" name="event_venue" type="text" class="form-control form-control-solid" required/>
                        </div>
                        <div class="mb-10">
                            <label for="event_capacity" class="required form-label">Etkinlik Kapasitesi</label>
                            <input id="event_capacity" value="{{ $event->event_capacity }}" name="event_capacity" type="number" class="form-control form-control-solid" required/>
                        </div>
                        <div class="mb-10">
                            <label for="event_last_apply_date" class="required form-label">Etkinlik Son Başvuru Tarihi</label>
                            <input id="event_last_apply_date" value="{{ $event->event_last_apply_date->format('Y-m-d\TH:i') }}" name="event_last_apply_date" type="datetime-local" class="form-control form-control-solid" required/>
                        </div>
                        <div class="mb-10">
                            <label for="event_start_date" class="required form-label">Etkinlik Başlangıç Tarihi</label>
                            <input id="event_start_date" value="{{ $event->event_start_date->format('Y-m-d\TH:i') }}" name="event_start_date" type="datetime-local" class="form-control form-control-solid" required/>
                        </div>
                        <div class="mb-10">
                            <label for="event_end_date" class="required form-label">Etkinlik Bitiş Tarihi</label>
                            <input id="event_end_date" value="{{ $event->event_end_date->format('Y-m-d\TH:i') }}" name="event_end_date" type="datetime-local" class="form-control form-control-solid" required/>
                        </div>
                        <div class="row mb-10">
                            <!--begin::Label-->
                            ...
                            <div class="col-lg-9 d-flex align-items-center">
                                <div class="form-check form-check-solid form-switch fv-row">
                                    <input name="event_is_visible" class="form-check-input w-45px h-30px"
                                           type="checkbox" id="event_is_visible" @if($event->event_is_visible) checked @endif />
                        ...
                        <div class="row mb-10">
                            ...
                            <div class="col-lg-9 d-flex align-items-center">
                                <div class="form-check form-check-solid form-switch fv-row">
                                    <input onclick="this.checked=!this.checked;" name="event_is_private" class="form-check-input w-45px h-30px"
                                           type="checkbox" id="event_is_private" @if($event->event_is_private) checked @endif />
                                    <label class="form-check-label" for="event_is_private"></label>
                                </div>
                            </div>
                            <!--begin::Label-->
                        </div>
                        <div class="row mb-10" id="event_attendees">
                            ...
                            <select class="form-control select2" id="event_private_users" name="event_private_users[]" multiple="multiple">
                                @foreach($users as $user)
                                    <option value="{{ $user->id }}" @if($event->guests->contains($user->id)) selected @endif>{{ $user->name }}</option>
                                @endforeach
                            </select>
                        </div>
                        <div class="row mb-10" id="hobby_club_select">
                            <label for="hobby_club" class="form-label">Hobi kulübü seçiniz.</label>
                            <select class="form-control" id="hobby_club" name="hobby_club" disabled>
                                <option value="" selected="" disabled="" hidden="">Lütfen hobi kulübü seçiniz</option>
                                @foreach($hobbies as $hobby)
                                    <option value="{{ $hobby->id }}" @if(optional($event->hobbies()->first())->id === $hobby->id) selected @endif>{{ $hobby->hobby_title }}</option>
                                @endforeach
                            </select>
                    ...
                    <div class="card-footer">
                        <div class="row">
                            ...
                            <div class="col-lg-6">
                                <button type="submit" class="btn btn-success mr-2">Etkinliği Düzenle</button>
